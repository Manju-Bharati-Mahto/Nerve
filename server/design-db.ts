...
export interface KraReport {
  ...
  team_joined_at: string | null;       // when the user joined the design team
...
export async function bootstrapDesignDatabase() {
  // The 'design' team must exist on ALREADY-SEEDED databases too (the global
  // team seeding only runs when the teams table is empty).
  await pool.query(
    `INSERT INTO teams (id, name, color, is_built_in) VALUES ('design', 'Design', 'violet', true)
     ON CONFLICT (id) DO NOTHING`,
  );

  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_work_categories (
      id TEXT PRIMARY KEY,
      name TEXT NOT NULL,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_work_sub_categories (
      id TEXT PRIMARY KEY,
      category_id TEXT NOT NULL REFERENCES design_work_categories(id) ON DELETE CASCADE,
      name TEXT NOT NULL,
      sort_order INTEGER NOT NULL DEFAULT 0,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_daily_reports (
      id TEXT PRIMARY KEY,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_daily_report_rows (
      id TEXT PRIMARY KEY,
      report_id TEXT NOT NULL REFERENCES design_daily_reports(id) ON DELETE CASCADE,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_kra_parameters (
      id TEXT PRIMARY KEY,
      name TEXT NOT NULL,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_self_appraisals (
      id TEXT PRIMARY KEY,
      user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_peer_markings (
      id TEXT PRIMARY KEY,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_admin_kra_scores (
      id TEXT PRIMARY KEY,
      user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      month INTEGER NOT NULL CHECK (month BETWEEN 1 AND 12),
      year INTEGER NOT NULL,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_peer_marking_settings (
      id TEXT PRIMARY KEY,
  ...
  await pool.query(`ALTER TABLE design_projects ADD COLUMN IF NOT EXISTS type_of_work TEXT NOT NULL DEFAULT ''`);
  await pool.query(`ALTER TABLE design_projects ADD COLUMN IF NOT EXISTS sub_category TEXT NOT NULL DEFAULT ''`);
  await pool.query(`ALTER TABLE design_projects ADD COLUMN IF NOT EXISTS specific_work TEXT NOT NULL DEFAULT ''`);
  ...
  await pool.query(`ALTER TABLE design_projects ADD COLUMN IF NOT EXISTS assigned_lead_id TEXT REFERENCES users(id) ON DELETE SET NULL`);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_project_assignments (
      id TEXT PRIMARY KEY,
  ...
  await pool.query(`ALTER TABLE design_project_assignments ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'pending'`);
  await pool.query(`ALTER TABLE design_project_assignments ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ`);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_report_row_comments (
      id TEXT PRIMARY KEY,
      row_id TEXT NOT NULL REFERENCES design_daily_report_rows(id) ON DELETE CASCADE,
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_design_brrc_row_id ON design_report_row_comments(row_id)`);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_designs (
      id TEXT PRIMARY KEY,
      title TEXT NOT NULL,
      description TEXT NOT NULL DEFAULT '',
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_design_votes (
      id TEXT PRIMARY KEY,
      design_id TEXT NOT NULL REFERENCES design_designs(id) ON DELETE CASCADE,
      user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS design_leaves (
      id TEXT PRIMARY KEY,
      user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  ...
  // ── Migration: stopwatch fields on design_daily_report_rows ─────────────────────
  await pool.query(`ALTER TABLE design_daily_report_rows ADD COLUMN IF NOT EXISTS stopwatch_status TEXT NOT NULL DEFAULT 'idle'`);
  await pool.query(`ALTER TABLE design_daily_report_rows ADD COLUMN IF NOT EXISTS elapsed_seconds INTEGER NOT NULL DEFAULT 0`);
  await pool.query(`ALTER TABLE design_daily_report_rows ADD COLUMN IF NOT EXISTS stopwatch_started_at TIMESTAMPTZ`);
  await pool.query(`ALTER TABLE design_daily_report_rows ADD COLUMN IF NOT EXISTS carried_over_from_row_id TEXT`);
  ...
  // pause/finish save.
  await pool.query(`ALTER TABLE design_daily_report_rows ADD COLUMN IF NOT EXISTS last_paused_at TIMESTAMPTZ`);

  // ── Migration: manual penalty on design_admin_kra_scores ────────────────────────
  await pool.query(`ALTER TABLE design_admin_kra_scores ADD COLUMN IF NOT EXISTS manual_penalty_percent NUMERIC(5,2) NOT NULL DEFAULT 0`);
  await pool.query(`ALTER TABLE design_admin_kra_scores ADD COLUMN IF NOT EXISTS manual_penalty_reason TEXT NOT NULL DEFAULT ''`);

  // ── Migration: full total-penalty override on design_admin_kra_scores ───────────
  // NULL means "no override; use auto + manual". A number replaces both.
  await pool.query(`ALTER TABLE design_admin_kra_scores ADD COLUMN IF NOT EXISTS total_penalty_override NUMERIC(5,2)`);
  await pool.query(`ALTER TABLE design_admin_kra_scores ADD COLUMN IF NOT EXISTS total_penalty_override_reason TEXT NOT NULL DEFAULT ''`);

  // ── Migration: half-day + datetime range on design_leaves ──────────────
  await pool.query(`ALTER TABLE design_leaves ADD COLUMN IF NOT EXISTS start_at TIMESTAMPTZ`);
  await pool.query(`ALTER TABLE design_leaves ADD COLUMN IF NOT EXISTS end_at TIMESTAMPTZ`);
  await pool.query(`ALTER TABLE design_leaves ADD COLUMN IF NOT EXISTS is_half_day BOOLEAN NOT NULL DEFAULT false`);
  await pool.query(`ALTER TABLE design_leaves ADD COLUMN IF NOT EXISTS half_day_period TEXT`);
  // Backfill start_at / end_at for legacy single-date rows
  await pool.query(`
    UPDATE design_leaves
       SET start_at = (leave_date::timestamp + INTERVAL '9 hours'),
           end_at   = (leave_date::timestamp + INTERVAL '17 hours')
     WHERE start_at IS NULL
  `);

  await seedDesignDefaults();
}

async function seedDesignDefaults() {
  const catCount = await pool.query<{ count: number }>(
    `SELECT COUNT(*)::int AS count FROM design_work_categories`
  );
  if (catCount.rows[0].count === 0) {
    const categories = [
      ...
      {
        id: "cat-venue-design", name: "Venue Design", order: 3,
        subs: ["Auditorium Design", "Ground Design"],
      },
      {
      ...
      { id: "cat-design-marketing", name: "Design and Marketing Material Design", order: 8, subs: [] },
      { id: "cat-others",             name: "Others",                                 order: 9, subs: [] },
    ];
    for (const cat of categories) {
      await pool.query(
        `INSERT INTO design_work_categories (id, name, sort_order) VALUES ($1, $2, $3)`,
        [cat.id, cat.name, cat.order]
      );
      let subOrder = 1;
      for (const sub of cat.subs) {
        await pool.query(
          `INSERT INTO design_work_sub_categories (id, category_id, name, sort_order, is_others) VALUES ($1, $2, $3, $4, false)`,
          [generateId("wsc"), cat.id, sub, subOrder++]
        );
      }
      if (cat.id !== "cat-others") {
        await pool.query(
          `INSERT INTO design_work_sub_categories (id, category_id, name, sort_order, is_others) VALUES ($1, $2, 'Others', 999, true)`,
  ...
  const kraCount = await pool.query<{ count: number }>(
    `SELECT COUNT(*)::int AS count FROM design_kra_parameters`
  );
  if (kraCount.rows[0].count === 0) {
    ...
    for (let i = 0; i < params.length; i++) {
      await pool.query(
        `INSERT INTO design_kra_parameters (id, name, description, max_score, sort_order) VALUES ($1, $2, $3, 10, $4)`,
  ...
  await pool.query(`
    INSERT INTO design_peer_marking_settings (id, is_enabled)
...
export async function listWorkCategories(): Promise<WorkCategory[]> {
  const cats = await pool.query<CatRow>(
    `SELECT * FROM design_work_categories ORDER BY sort_order ASC, name ASC`
  );
  const subs = await pool.query<SubCatRow>(
    `SELECT * FROM design_work_sub_categories ORDER BY is_others ASC, sort_order ASC, name ASC`
  );
...
export async function createWorkCategory(name: string): Promise<WorkCategory> {
  const maxRes = await pool.query<{ max: number }>(
    `SELECT COALESCE(MAX(sort_order), 0) AS max FROM design_work_categories`
  );
...
export async function updateWorkCategory(id: string, name: string): Promise<boolean> {
  const res = await pool.query(
    `UPDATE design_work_categories SET name = $2 WHERE id = $1`, [id, name]
  );
...
export async function deleteWorkCategory(id: string): Promise<{ usageCount: number }> {
  const usage = await pool.query<{ count: number }>(
    `SELECT COUNT(*)::int AS count FROM design_daily_report_rows drr
     JOIN design_daily_reports dr ON drr.report_id = dr.id
     WHERE drr.type_of_work = (SELECT name FROM design_work_categories WHERE id = $1)`,
    [id]
  );
  await pool.query(`DELETE FROM design_work_categories WHERE id = $1`, [id]);
...
export async function createWorkSubCategory(categoryId: string, name: string): Promise<WorkSubCategory> {
  const maxRes = await pool.query<{ max: number }>(
    `SELECT COALESCE(MAX(sort_order), 0) AS max FROM design_work_sub_categories WHERE category_id = $1 AND NOT is_others`,
    [categoryId]
  );
  const id = generateId("wsc");
  const result = await pool.query<SubCatRow>(
    `INSERT INTO design_work_sub_categories (id, category_id, name, sort_order, is_others)
...
export async function updateWorkSubCategory(id: string, name: string): Promise<boolean> {
  const res = await pool.query(
    `UPDATE design_work_sub_categories SET name = $2 WHERE id = $1 AND NOT is_others`, [id, name]
  );
  return (res.rowCount ?? 0) > 0;
}
...
export async function deleteWorkSubCategory(id: string): Promise<{ usageCount: number }> {
  const usage = await pool.query<{ count: number }>(
    `SELECT COUNT(*)::int AS count FROM design_daily_report_rows drr
     JOIN design_daily_reports dr ON drr.report_id = dr.id
     WHERE drr.sub_category = (SELECT name FROM design_work_sub_categories WHERE id = $1)`,
    [id]
  );
  await pool.query(`DELETE FROM design_work_sub_categories WHERE id = $1 AND NOT is_others`, [id]);
  return { usageCount: usage.rows[0].count };
}

export async function reorderWorkCategories(orderedIds: string[]): Promise<void> {
  for (let i = 0; i < orderedIds.length; i++) {
    await pool.query(`UPDATE design_work_categories SET sort_order = $2 WHERE id = $1`, [orderedIds[i], i + 1]);
  }
}
...
async function fetchReportRows(reportId: string): Promise<DailyReportRow[]> {
  const res = await pool.query<ReportRowDb>(
    `SELECT * FROM design_daily_report_rows WHERE report_id = $1 ORDER BY sr_no ASC`,
    [reportId]
  );
  return res.rows;
}
...
export async function getOrCreateDailyReport(userId: string, date: string): Promise<DailyReport> {
  const existing = await pool.query<ReportDbRow>(
    `SELECT * FROM design_daily_reports WHERE user_id = $1 AND report_date = $2::date`,
    [userId, date]
  );
  if (existing.rows[0]) {
  ...
  const result = await pool.query<ReportDbRow>(
    `INSERT INTO design_daily_reports (id, user_id, report_date) VALUES ($1, $2, $3::date) RETURNING *`,
    [id, userId, date]
  );
...
async function carryOverPausedRows(
  ...
  const lockCheck = await pool.query<{ is_locked: boolean }>(
    `SELECT is_locked FROM design_daily_reports WHERE id = $1`, [todayReportId]
  );
  if (lockCheck.rows[0]?.is_locked) return;

  const prevReport = await pool.query<{ id: string }>(
    `SELECT id FROM design_daily_reports
  ...
  const pausedRows = await pool.query<ReportRowDb>(
    `SELECT * FROM design_daily_report_rows
  ...
  for (const src of pausedRows.rows) {
    ...
    await pool.query(
      `INSERT INTO design_daily_report_rows
...
// Compact "Hh Mm Ss" representation — mirrors src/lib/design-types.ts
...
export async function saveReportRows(reportId: string, userId: string, rows: SaveRowInput[]): Promise<DailyReportRow[] | null> {
  const report = await pool.query<ReportDbRow>(
    `SELECT * FROM design_daily_reports WHERE id = $1 AND user_id = $2`,
    [reportId, userId]
  );
  ...
  await pool.query(`DELETE FROM design_daily_report_rows WHERE report_id = $1`, [reportId]);
...
export async function submitDailyReport(reportId: string, userId: string): Promise<DailyReport | null> {
  ...
  if (!report.rows[0] || report.rows[0].is_locked) return null;
  const res = await pool.query<ReportDbRow>(
    `UPDATE design_daily_reports SET is_locked = true, submitted_at = NOW() WHERE id = $1 RETURNING *`,
    [reportId]
  );
...
async function finalizeReportRowsForSubmit(reportId: string): Promise<void> {
  const res = await pool.query<ReportRowDb>(
    `SELECT * FROM design_daily_report_rows WHERE report_id = $1`,
    [reportId]
  );
  for (const row of res.rows) {
    ...
    if (newStatus !== status || elapsed !== (row.elapsed_seconds ?? 0) || newTimeTaken !== row.time_taken) {
      await pool.query(
        `UPDATE design_daily_report_rows
...
export async function autoSubmitOverdueReports(): Promise<number> {
  ...
  for (const r of overdue.rows) {
    ...
    const upd = await pool.query(
      `UPDATE design_daily_reports SET is_locked = true, submitted_at = NOW()
...
export async function autoPauseRunningStopwatches(): Promise<number> {
  const overdue = await pool.query<{
    ...
    SELECT drr.id, drr.stopwatch_started_at, drr.elapsed_seconds
      FROM design_daily_report_rows drr
      JOIN design_daily_reports dr ON dr.id = drr.report_id
...
export async function listAllDailyReports(filters?: {
  ...
  // When specific userId(s) are given, filter directly; otherwise scope to design team.
...
export async function listKraParameters(): Promise<KraParameter[]> {
  const res = await pool.query<KraParamRow>(`SELECT * FROM design_kra_parameters ORDER BY sort_order ASC`);
  return res.rows;
}

export async function getPeerMarkingEnabled(): Promise<boolean> {
  const res = await pool.query<{ is_enabled: boolean }>(
    `SELECT is_enabled FROM design_peer_marking_settings WHERE id = 'singleton'`
  );
...
export async function togglePeerMarking(enabled: boolean, adminId: string): Promise<void> {
  await pool.query(
    `INSERT INTO design_peer_marking_settings (id, is_enabled, toggled_by, toggled_at)
...
export async function getSelfAppraisal(userId: string, month: number, year: number): Promise<SelfAppraisal | null> {
  const res = await pool.query<SelfAppraisalRow>(
    `SELECT * FROM design_self_appraisals WHERE user_id = $1 AND month = $2 AND year = $3`,
    [userId, month, year]
  );
...
export async function submitSelfAppraisal(
  ...
  const res = await pool.query<SelfAppraisalRow>(
    `INSERT INTO design_self_appraisals (id, user_id, month, year, scores)
...
export async function listAllSelfAppraisals(month: number, year: number): Promise<(SelfAppraisal & { user_name: string })[]> {
  const res = await pool.query<SelfAppraisalRow & { user_name: string }>(
    `SELECT sa.*, u.full_name AS user_name
     FROM design_self_appraisals sa
...
export async function getCompletedPeerMarkings(reviewerId: string, month: number, year: number): Promise<string[]> {
  const res = await pool.query<{ reviewee_id: string }>(
    `SELECT reviewee_id FROM design_peer_markings WHERE reviewer_id = $1 AND month = $2 AND year = $3`,
...
export async function submitPeerMarking(
  ...
  const existing = await pool.query(
    `SELECT id FROM design_peer_markings WHERE reviewer_id = $1 AND reviewee_id = $2 AND month = $3 AND year = $4`,
  ...
  const res = await pool.query<PeerMarkingRow>(
    `INSERT INTO design_peer_markings (id, reviewer_id, reviewee_id, month, year, scores)
...
export async function getAdminKraScore(userId: string, month: number, year: number): Promise<AdminKraScore | null> {
  const res = await pool.query<AdminKraRow>(
    `SELECT * FROM design_admin_kra_scores WHERE user_id = $1 AND month = $2 AND year = $3`,
    [userId, month, year]
  );
...
export async function setAdminKraScore(
  ...
  const res = await pool.query<AdminKraRow>(
    `INSERT INTO design_admin_kra_scores (id, user_id, month, year, scores, updated_at)
...
// Creates the design_admin_kra_scores row if it doesn't exist yet (so admins can apply
...
export async function setAdminManualPenalty(
  ...
  const id = generateId("aks");
  const res = await pool.query<AdminKraRow>(
    `INSERT INTO design_admin_kra_scores
...
export async function finalPushKra(
  ...
  const res = await pool.query<AdminKraRow>(
    `UPDATE design_admin_kra_scores
...
// off — Saturday is a working day for the design team.
...
async function computeMissedReportPenalty(
  ...
  const leaveRes = await pool.query<{ leave_date: string; is_half_day: boolean }>(
    `SELECT leave_date, is_half_day FROM design_leaves
  ...
  const subRes = await pool.query<{ count: string }>(
    `SELECT COUNT(*)::int AS count FROM design_daily_reports
...
export async function getAdminKraDashboard(month: number, year: number): Promise<KraReport[]> {
  const users = await pool.query<{ id: string }>(
    `SELECT id FROM users WHERE team = 'design' AND role != 'super_admin' ORDER BY full_name ASC`
  );
...
export async function listDesignDesigns(filters?: {
  search?: string; category?: string; uploaderId?: string;
  dateFrom?: string; dateTo?: string;
}, currentUserId?: string): Promise<DesignDesign[]> {
  ...
  const result = await pool.query<DesignRow>(
    `SELECT d.*,
       (SELECT COUNT(*)::int FROM design_design_votes WHERE design_id = d.id AND vote_type = 'up') AS upvotes,
       (SELECT COUNT(*)::int FROM design_design_votes WHERE design_id = d.id AND vote_type = 'down') AS downvotes,
       (SELECT vote_type FROM design_design_votes WHERE design_id = d.id AND user_id = $1) AS user_vote
     FROM design_designs d ${where} ORDER BY d.created_at DESC`,
    params
  );
...
export async function createDesignDesign(
  title: string, description: string, category: string,
  tags: string[], imageUrl: string, uploaderId: string, uploaderName: string
): Promise<DesignDesign> {
  const id = generateId("des");
  const result = await pool.query<DesignRow>(
    `INSERT INTO design_designs (id, title, description, category, tags, image_url, uploader_id, uploader_name)
...
export async function deleteDesignDesign(id: string): Promise<string | null> {
  const res = await pool.query<{ image_url: string }>(
    `DELETE FROM design_designs WHERE id = $1 RETURNING image_url`, [id]
  );
  return res.rows[0]?.image_url ?? null;
}

export async function getDesignDesignById(id: string): Promise<DesignDesign | null> {
  const res = await pool.query<DesignRow>(
    `SELECT d.*,
       (SELECT COUNT(*)::int FROM design_design_votes WHERE design_id = d.id AND vote_type = 'up') AS upvotes,
       (SELECT COUNT(*)::int FROM design_design_votes WHERE design_id = d.id AND vote_type = 'down') AS downvotes,
       NULL::text AS user_vote
     FROM design_designs d WHERE d.id = $1`,
    [id]
  );
  if (!res.rows[0]) return null;
  const r = res.rows[0];
...
export async function castDesignVote(
  ...
  if (voteType === null) {
    await pool.query(
      `DELETE FROM design_design_votes WHERE design_id = $1 AND user_id = $2`,
      [designId, userId]
    );
  } else {
    const id = generateId("vote");
    await pool.query(
      `INSERT INTO design_design_votes (id, design_id, user_id, vote_type)
       VALUES ($1, $2, $3, $4)
  ...
  const counts = await pool.query<{ upvotes: string; downvotes: string }>(
    `SELECT
       (SELECT COUNT(*)::int FROM design_design_votes WHERE design_id = $1 AND vote_type = 'up') AS upvotes,
       (SELECT COUNT(*)::int FROM design_design_votes WHERE design_id = $1 AND vote_type = 'down') AS downvotes`,
    [designId]
  );
  return {
...
export interface DesignPortalStats {
  designs_count: number;
  projects_count: number;
  today_submitted: number;
  today_total: number;
  recent_designs: Pick<DesignDesign, 'id' | 'title' | 'image_url' | 'uploader_name' | 'created_at' | 'upvotes' | 'downvotes'>[];
}

export async function getDesignPortalStats(): Promise<DesignPortalStats> {
  const [designs, projects, reportStatus, recentRaw] = await Promise.all([
    pool.query<{ count: string }>(`SELECT COUNT(*)::int AS count FROM design_designs`),
    pool.query<{ count: string }>(`SELECT COUNT(*)::int AS count FROM design_projects`),
    pool.query<{ total: string; submitted: string }>(
      `SELECT
         COUNT(DISTINCT u.id)::int AS total,
         COUNT(DISTINCT dr.user_id)::int AS submitted
       FROM users u
       LEFT JOIN design_daily_reports dr
         ON dr.user_id = u.id AND dr.report_date = CURRENT_DATE AND dr.is_locked = true
       WHERE u.team = 'design' AND u.role IN ('user','sub_admin')`
    ),
    pool.query<{ id: string; title: string; image_url: string; uploader_name: string; created_at: string }>(
      `SELECT d.id, d.title, d.image_url, d.uploader_name, d.created_at
       FROM design_designs d ORDER BY d.created_at DESC LIMIT 4`
    ),
  ]);
 ...
 */
export async function listDesignProjects(forUserId?: string): Promise<DesignProject[]> {
  ...
  const assignments = await pool.query<{ project_id: string; user_id: string; status: string; completed_at: string | null }>(
    `SELECT project_id, user_id, status, completed_at FROM design_project_assignments`
  );
  const mapped = projects.rows.map(p => {
    const mine = assignments.rows.filter(a => a.project_id === p.id);
    return {
      ...p,
      status: p.status as DesignProject["status"],
...
export async function createDesignProject(
  ...
  await pool.query(
    `INSERT INTO design_projects (id, name, description, deadline, created_by, type_of_work, sub_category, specific_work, assigned_lead_id)
  ...
  for (const userId of assignedUserIds) {
    await pool.query(
      `INSERT INTO design_project_assignments (id, project_id, user_id, assigned_by)
       VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING`,
      [generateId("pa"), id, userId, createdBy]
    );
  }
  const all = await listDesignProjects();
 ...
 */
export async function completeProjectAssignment(projectId: string, userId: string): Promise<DesignProject | null> {
  const upd = await pool.query(
    `UPDATE design_project_assignments SET status = 'completed', completed_at = NOW()
      WHERE project_id = $1 AND user_id = $2`,
    [projectId, userId]
  );
  if ((upd.rowCount ?? 0) === 0) return null;
  const remaining = await pool.query<{ n: string }>(
    `SELECT COUNT(*) AS n FROM design_project_assignments WHERE project_id = $1 AND status <> 'completed'`,
    [projectId]
  );
  if (Number(remaining.rows[0].n) === 0) {
    await pool.query(`UPDATE design_projects SET status = 'completed' WHERE id = $1`, [projectId]);
  }
  const all = await listDesignProjects();
...
export async function updateDesignProject(
  ...
  const res = await pool.query<ProjectRow>(
    `UPDATE design_projects SET name=$2, description=$3, deadline=$4, status=$5,
  ...
  await pool.query(`DELETE FROM design_project_assignments WHERE project_id = $1`, [id]);
  for (const userId of assignedUserIds) {
    await pool.query(
      `INSERT INTO design_project_assignments (id, project_id, user_id, assigned_by)
       VALUES ($1, $2, $3, $4)`,
...
export async function deleteDesignProject(id: string): Promise<boolean> {
  const res = await pool.query(
    `DELETE FROM design_projects WHERE id = $1`, [id]
  );
  return (res.rowCount ?? 0) > 0;
}
...
export async function createAssignedReportRow(opts: {
  ...
  const dup = await pool.query(
    `SELECT 1 FROM design_daily_report_rows
  ...
  const next = await pool.query<{ n: number }>(
    `SELECT COALESCE(MAX(sr_no),0)+1 AS n FROM design_daily_report_rows WHERE report_id=$1`,
    [report.id]
  );
  await pool.query(
    `INSERT INTO design_daily_report_rows
...
export async function listRowComments(rowIds: string[]): Promise<ReportRowComment[]> {
  ...
  const res = await pool.query<RowCommentDbRow>(
    `SELECT c.*, u.full_name AS author_name
       FROM design_report_row_comments c
...
export async function createRowComment(
  ...
  await pool.query(
    `INSERT INTO design_report_row_comments (id, row_id, author_id, body)
     VALUES ($1, $2, $3, $4)`,
...
export async function updateRowComment(
  ...
  const res = await pool.query<RowCommentDbRow>(
    `UPDATE design_report_row_comments
...
export async function deleteRowComment(id: string, authorId: string): Promise<boolean> {
  const res = await pool.query(
    `DELETE FROM design_report_row_comments WHERE id = $1 AND author_id = $2`,
    [id, authorId]
  );
  return (res.rowCount ?? 0) > 0;
}
...
function mapLeave(row: LeaveDbRow): DesignLeave {
  ...
  return {
    ...
    half_day_period: (row.half_day_period === 'first' || row.half_day_period === 'second')
      ...
      : null,
    reason: row.reason,
    status: row.status as DesignLeave['status'],
...
export async function getUserLeaves(userId: string): Promise<DesignLeave[]> {
  const res = await pool.query<LeaveDbRow>(
    `SELECT * FROM design_leaves WHERE user_id = $1 ORDER BY leave_date DESC`,
    [userId]
  );
  return res.rows.map(mapLeave);
}

export async function getAllLeaves(status?: string): Promise<DesignLeave[]> {
  let q = `SELECT l.*, u.full_name AS user_name, u.email AS user_email
           FROM design_leaves l
...
): Promise<DesignLeave | null> {
  const res = await pool.query<LeaveDbRow>(
    `UPDATE design_leaves SET status = $2, reviewed_by = $3, reviewed_at = NOW()
     WHERE id = $1 RETURNING *`,
...
export async function updateLeaveTransfer(
  ...
  const res = await pool.query<LeaveDbRow>(
    `UPDATE design_leaves SET transfer_date = $2
     WHERE id = $1 RETURNING *`,
...
export async function cancelLeave(leaveId: string, userId: string): Promise<boolean> {
  const res = await pool.query(
    `DELETE FROM design_leaves WHERE id = $1 AND user_id = $2 AND status = 'pending'`,
    [leaveId, userId]
  );
  return (res.rowCount ?? 0) > 0;
}

export async function getLeaveForDate(userId: string, date: string): Promise<DesignLeave | null> {
  const res = await pool.query<LeaveDbRow>(
    `SELECT * FROM design_leaves WHERE user_id = $1 AND leave_date = $2::date`,
    [userId, date]
  );
  return res.rows[0] ? mapLeave(res.rows[0]) : null;
}
