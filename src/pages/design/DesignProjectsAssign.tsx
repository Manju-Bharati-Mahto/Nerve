/**
 * Assign Projects — capability-gated tab (design:assign_projects).
 *
 * A design head grants this capability to certain leads (via Team Members →
...
import type { WorkCategory, DesignProject } from '@/lib/design-types'
...
export default function DesignProjectsAssign() {
  ...
  useEffect(() => { designApi.getCategories().then(r => setCategories(r.categories)).catch(() => {}) }, [])
  useEffect(() => { designApi.getProjects().then(r => setProjects(r.projects)).catch(() => {}) }, [])

  // "Assign to designers" lists the WHOLE design team — designers, leads and
  ...
  const designers = useMemo(() => users.filter(u =>
    u.team === 'design' && (u.role === 'user' || u.role === 'sub_admin' || u.role === 'task_owner' || u.role === 'task_manager') &&
    u.id !== user?.id
  ), [users, user?.id])

  // "Assign Lead" dropdown: supervisory profiles — the design head (admin),
  ...
  const leads = useMemo(() => users.filter(u =>
    u.team === 'design' && (u.role === 'admin' || u.role === 'sub_admin' || u.role === 'task_owner' || u.role === 'task_manager')
  ), [users])
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const { project } = await designApi.assignProject({
        name: name.trim(),
          ...
          <div>
            ...
            {designers.length === 0 ? (
              <p className="text-xs text-gray-400 border border-gray-100 rounded-xl px-3 py-3">
                No members to assign. {isAdmin ? 'Add design team members first.' : 'No design team members found yet.'}
              </p>
            ) : (
