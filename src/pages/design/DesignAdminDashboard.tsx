...
import { MONTHS } from '@/lib/design-types'
import type {
  WorkCategory, KraParameter, KraReport,
  AdminKraScore, PeerMarking, DesignLeave,
...
import DesignAdminOverview from './DesignAdminOverview'
import DesignAdminShell from './DesignAdminShell'
...
const NON_SUBMITTING_ROLES = new Set(['super_admin', 'admin', 'design_reports_admin'])
...
function ManageCategoriesTab() {
  ...
  async function addCategory() {
    ...
    if (!name) return
    try {
      await designApi.createCategory(name)
      setNewCatName('')
      load()
  ...
  async function saveCategory(id: string, name: string) {
    try {
      await designApi.updateCategory(id, name)
      setEditingCat(null)
      load()
  ...
  async function deleteCategory(id: string, name: string) {
    ...
    try {
      await designApi.deleteCategory(id)
      load()
  ...
  async function addSubCategory(catId: string) {
    ...
    if (!name) return
    try {
      await designApi.createSubCategory(catId, name)
  ...
  async function saveSubCategory(id: string, name: string) {
    try {
      await designApi.updateSubCategory(id, name)
      setEditingSub(null)
      load()
  ...
  async function deleteSubCategory(id: string, name: string) {
    ...
    try {
      await designApi.deleteSubCategory(id)
      load()
  ...
  async function moveCategory(id: string, dir: 'up' | 'down') {
    ...
    await designApi.reorderCategories(newOrder.map(c => c.id))
  }
...
function KraManagementTab({ designUsers }: { designUsers: { id: string; full_name: string; email: string; avatar_url: string | null }[] }) {
  ...
  const load = useCallback(() => {
    setLoading(true)
    Promise.all([
      designApi.getAdminKraDashboard(month, year),
      designApi.getKraParameters(),
      designApi.getPeerMarkingEnabled(),
    ])
      .then(([d, p, e]) => {
  ...
  useEffect(() => {
    ...
    designApi.getAdminScore(selectedUser, month, year)
      .then(r => setAdminScores(r.score?.scores || Object.fromEntries(params.map(p => [p.id, 5]))))
      .catch(() => {})
    setUserPeerLoading(true)
    designApi.getUserPeerMarkings(selectedUser, month, year)
  ...
  async function togglePeer(enabled: boolean) {
    try {
      await designApi.togglePeerMarking(enabled)
  ...
  async function saveAdminScore() {
    ...
    setAdminSaving(true)
    try {
      await designApi.setAdminScore(selectedUser, month, year, adminScores)
      load()
  ...
  async function saveManualPenalty() {
    ...
    try {
      await designApi.setAdminPenalty(selectedUser, month, year, pct, penaltyReason.trim())
      await load()
  ...
  async function saveTotalOverride() {
    ...
    try {
      await designApi.setTotalPenaltyOverride(selectedUser, month, year, pct, overrideReason.trim())
      await load()
  ...
  async function doFinalPush() {
    if (!selectedUser) return
    try {
      await designApi.finalPush(selectedUser, month, year)
  ...
  async function loadPeerMarkings() {
    designApi.getAllPeerMarkings(month, year)
  ...
  const kraMembers = useMemo(() => {
    const ids = new Set(designUsers.map(b => b.id))
    return dashboard.filter(r => ids.has(r.user_id))
  }, [dashboard, designUsers])
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {kraMembers.map(r => {
                ...
                const u = designUsers.find(b => b.id === r.user_id)
            ...
            {!selectedUser ? (
              ...
              <div className="hub-card space-y-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    {(() => {
                      const su = designUsers.find(b => b.id === selectedReport.user_id)
...
function LeaveManagementTab() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    designApi.getLeaves(statusFilter === 'all' ? undefined : statusFilter)
  ...
  async function handleReview(id: string, status: 'approved' | 'rejected') {
    try {
      const res = await designApi.reviewLeave(id, status)
  ...
  async function handleTransferSave(id: string) {
    try {
      const res = await designApi.updateLeaveTransfer(id, transferVal || null)
  ...
  const statusBadgeCls = (s: DesignLeave['status']) =>
...
const SECTION_BY_PATH: Record<string, AdminSection> = {
  ...
  '/design/kra':            'kra',
  '/design/leaves':         'leaves',
  '/design/categories':     'categories',
  '/design/leave-calendar': 'leave-calendar',
}
...
export default function DesignAdminDashboard() {
  ...
  // collaboration map). Roles like `admin` and `design_reports_admin`
  // exist to manage the team, not to log work, so they're excluded.
  const designUsers = useMemo(() =>
    users.filter(u => u.team === 'design' && !NON_SUBMITTING_ROLES.has(u.role))
...
function LeaveCalendarTab() {
  ...
  useEffect(() => { designApi.getLeaves().then(r => setLeaves(r.leaves)).catch(() => {}) }, [])
  ...
  const byDate = useMemo(() => {
    const m = new Map<string, { submitted: number; approved: number; items: DesignLeave[] }>()
    const add = (day: string, l: DesignLeave) => {
