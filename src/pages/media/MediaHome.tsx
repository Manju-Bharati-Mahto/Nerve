  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-extrabold font-serif" style={{ color: MEDIA_GREEN }}>
            {new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}
          </h1>
          <p className="text-sm text-gray-500">{isLead ? 'Department overview' : 'Your production overview'}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => navigate('/media/projects?new=1')}
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-semibold border border-gray-200 text-gray-600 hover:bg-gray-50">
            <FolderKanban className="w-4 h-4" /> New Project
          </button>
          <button onClick={load} title="Refresh" className="p-2 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50">
              ...
              <div className="space-y-2">
                {overdue.map(d => (
                  <Link key={d.id} to={`/media/projects/${d.project_id}`}
                    className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 transition-colors">
                ...
                {isLead && dash.reviewQueue.map(r => (
                  <Link key={r.id} to="/media/reports"
                    className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl bg-amber-50 hover:bg-amber-100 transition-colors">
                    <div className="min-w-0">
            ...
            {dash.runningProjects.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50">
                {dash.runningProjects.map(p => (
                  <Link key={p.id} to={`/media/projects/${p.id}`} className="flex items-center gap-3 py-2.5 hover:bg-gray-50 rounded-lg px-2 -mx-2">
...
function Stat({ label, value, sub, accent, icon: Icon }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
      ...
      <div className="min-w-0">
        <p className="text-xl font-extrabold leading-none tabular-nums" style={{ color: accent }}>{value}</p>
