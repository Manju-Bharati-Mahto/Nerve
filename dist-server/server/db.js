...
import { BUILT_IN_TEAMS, DEFAULT_SUPER_ADMIN_EMAIL, LEGACY_SUPER_ADMIN_PASSWORD, SEED_ENTRIES, SEED_USERS, SUPER_ADMIN_SEED_ID, isSeedSuperAdmin, } from "./seed.js";
...
async function seedDefaults() {
    const teamCount = await pool.query(`SELECT COUNT(*)::int AS count FROM teams`);
    if (teamCount.rows[0].count === 0) {
        for (const team of BUILT_IN_TEAMS) {
            await pool.query(`INSERT INTO teams (id, name, color, is_built_in) VALUES ($1, $2, $3, $4)`, [team.id, team.name, team.color, team.isBuiltIn]);
        }
    }
    ...
    if (userCount.rows[0].count === 0) {
        for (const user of SEED_USERS) {
            ...
            await pool.query(`INSERT INTO users (id, full_name, email, department, role, team, managed_by, password_hash)
    ...
    const entryCount = await pool.query(`SELECT COUNT(*)::int AS count FROM entries`);
...
async function rotateLegacySuperAdminCredentials() {
    const result = await pool.query(`SELECT * FROM users WHERE id = $1 LIMIT 1`, [SUPER_ADMIN_SEED_ID]);
    const superAdmin = result.rows[0];
    if (!superAdmin || superAdmin.role !== "super_admin")
        return;
    const shouldUpdateEmail = superAdmin.email === DEFAULT_SUPER_ADMIN_EMAIL &&
        superAdmin.email !== config.superAdminEmail;
    const shouldUpdatePassword = await verifyPassword(LEGACY_SUPER_ADMIN_PASSWORD, superAdmin.password_hash);
...
export async function createUser(input) {
    ...
    const result = await pool.query(`INSERT INTO users (id, full_name, email, department, role, team, managed_by, password_hash)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     RETURNING *`, [
        generateId("u"),
        input.full_name,
...
export async function createEntry(input) {
    const result = await pool.query(`INSERT INTO entries (
      id, title, dept, type, body, priority, entry_date, created_by, tags,
...
export async function listTeams() {
    const result = await pool.query(`SELECT * FROM teams ORDER BY is_built_in DESC, name ASC`);
    return result.rows.map(mapTeam);
}
export async function createTeam(input) {
    const id = input.name.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
    const result = await pool.query(`INSERT INTO teams (id, name, color, is_built_in)
     VALUES ($1, $2, $3, false)
     RETURNING *`, [id, input.name, input.color]);
...
export async function listBrandingRows() {
    const result = await pool.query(`SELECT * FROM branding_rows ORDER BY created_at ASC`);
    return result.rows.map(mapBrandingRow);
}
export async function createBrandingRow(input) {
    const result = await pool.query(`INSERT INTO branding_rows (
...
export async function updateBrandingRow(id, input) {
    const current = await pool.query(`SELECT * FROM branding_rows WHERE id = $1`, [id]);
    if (!current.rows[0])
        return null;
    const row = current.rows[0];
    const result = await pool.query(`UPDATE branding_rows
     SET category = $2,
         sub_category = $3,
         time_taken = $4,
         team_member = $5,
...
export async function deleteBrandingRow(id) {
    await pool.query(`DELETE FROM branding_rows WHERE id = $1`, [id]);
}
export async function getBootstrapData(includeBrandingRows) {
    const [entries, users, teams, brandingRows] = await Promise.all([
        listEntries(),
        listUsers(),
        listTeams(),
        includeBrandingRows ? listBrandingRows() : Promise.resolve([]),
    ]);
