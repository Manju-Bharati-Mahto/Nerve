...
import { bootstrapDatabase, createBrandingRow, createEntry, createTeam, createUser, deleteBrandingRow, deleteEntry, deleteTeam, deleteUser, getBootstrapData, getUserByEmail, getUserById, listBrandingRows, listEntries, listTeams, listUsers, pool, updateBrandingRow, updateUser, } from "./db.js";
...
function isBrandingManager(role, team) {
    return role === "super_admin" || (team === "branding" && (role === "admin" || role === "sub_admin" || role === "user"));
}
function canCreateManagedUser(actor, payload) {
    if (!actor)
        return false;
...
app.post("/api/auth/login", asyncHandler(async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "Invalid login payload.");
...
app.post("/api/entries", asyncHandler(async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "Invalid entry payload.");
    const currentUser = res.locals.currentUser;
...
app.post("/api/users", asyncHandler(async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "Invalid user payload.");
    const currentUser = res.locals.currentUser;
...
app.patch("/api/users/:id", asyncHandler(async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "Invalid user update payload.");
...
app.post("/api/teams", asyncHandler(async (req, res, next) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "Invalid team payload.");
    try {
...
app.post("/api/branding-rows", asyncHandler(async (req, res) => {
    ...
    if (!parsed.success)
        return sendError(res, 400, "Invalid branding row payload.");
...
app.patch("/api/branding-rows/:id", asyncHandler(async (req, res) => {
    ...
    const parsed = brandingRowSchema.safeParse(req.body);
    if (!parsed.success)
        return sendError(res, 400, "Invalid branding row update payload.");
    const brandingRow = await updateBrandingRow(rowId, parsed.data);
    if (!brandingRow)
        return sendError(res, 404, "Branding row not found.");
